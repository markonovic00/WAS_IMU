...
#define RAD_TO_DEG_WAS 57.29577951308232087679815481410
...
Adafruit_Mahony WASFilter;  // fastest/smalleset
...
void setup() {
  ...
  if (useBNOWAS)
    WASFilter.begin(FILTER_UPDATE_RATE_HZ);
...
void loop() {
  ...
  if ((systick_millis_count - READ_BNO_TIME) > REPORT_INTERVAL && useBNO08x) {
    ...
    readBNO();
    //BNOSteerAngle(); // This should be working (No compensation for roll)
    BNOSteerAngleFusion(); // Not tested, compensation added using fusion algorithm 
  }
